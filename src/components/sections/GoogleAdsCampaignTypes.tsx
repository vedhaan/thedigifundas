...
const campaigns = [
  { icon: faMagnifyingGlass, title: "Search Campaigns" },
  { icon: faImage, title: "Display Campaigns" },
  { icon: faCartShopping, title: "Shopping Campaigns" },
  { icon: faYoutube, title: "Video Campaigns" },
  { icon: faArrowsRotate, title: "Remarketing Campaigns" },
  { icon: faLocationDot, title: "Local Campaigns" },
];

function Pill({ icon, title }: { icon: typeof faMagnifyingGlass; title: string }) {
  return (
...
export default function GoogleAdsCampaignTypes() {
  const row = [...campaigns, ...campaigns];

  return (
    <Section id="google-ads-campaign-types" bgSecondary className="relative overflow-hidden">
      ...
      <Container className="relative !max-w-[1280px] 2xl:!max-w-[1600px]">
        ...
        {/* MOBILE / TABLET — separate, simpler code path, no marquee mechanics */}
        <div className="mx-auto mt-12 flex max-w-md flex-col gap-3 lg:hidden">
          {campaigns.map((c, i) => (
            ...
            >
              ...
              <span className="font-heading text-sm font-semibold text-text-primary">{c.title}</span>
            </motion.div>
          ))}
        </div>
      </Container>

      {/* DESKTOP marquee only — pure CSS loop, no JS scroll tracking */}
      <div
      ...
      <style>{`
        .marquee-track {
          animation: google-ads-marquee 26s linear infinite;
        }
        .marquee-track:hover {
          animation-play-state: paused;
        }
        @keyframes google-ads-marquee {
